    ...
    public function processarAction()
    {
        ...
        if (isset($response['status']) && $response['status'] == 200) {
            ...
            if ($dados['metodo'] == 'pix' && !isset($_FILES['comprovanteAnexo']) && $cadastro['tipoCadastro'] != 1) {
                ...
                $response['data']['url'] = $urlHelper->fromRoute('criar-anuncio/anuncio/pagamento/metodos', $routeParams) . '?idVeiculo=' . $idVeiculo . '&test=1&code=' . urlencode($response['data']['qr_code']) . '&idPagamento=' . md5($response['data']['idPagamento']);
            }
