        ...
        if (licensePlate.length == 7) {
            ...
            $.ajax(settings).done(function (response) {
                if (response.status == 200) {
                    ...
                    $('.table-license-plate-api-model').text(
                        response.data[0].historicoCarro.dados_veiculo.modelo,
                    );
                    $('.table-license-plate-api-color').text(
                        response.data[0].historicoCarro.dados_veiculo.cor,
                    );
                    $('.table-license-plate-api-fuel').text(
                        response.data[0].historicoCarro.dados_veiculo.combustivel,
                    );
                    $('.table-license-plate-api-city').text(
                        response.data[0].historicoCarro.dados_veiculo.cidade,
                    );
                    $('.table-license-plate-api-state').text(
                        response.data[0].historicoCarro.dados_veiculo.estado,
                    );
                    if (!response.data[0].historicoCarro.fipe) {
                        $('.table-license-plate-api-fipe-price').text('Indisponível');
                    } else {
                        $('.table-license-plate-api-fipe-price').text(
                            response.data[0].historicoCarro.fipe.valor_fipe,
                        );
                    }
